...
const convert24hTo12h = (hour24: number): number => {
...
const parseTimeId = (timeId: string) => {
  ...
  const hour12 = convert24hTo12h(hour24)
...
export const validateTimeInput = (timeId: string, userInput: string): boolean => {
  const validExpressions = generateValidExpressions(timeId)
  const normalizedInput = normalizeInput(userInput)
  
  return validExpressions.some(expr => expr === normalizedInput)
}
