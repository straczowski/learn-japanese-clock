...
export const TimeDisplay = ({ timeId, clockDisplayMode }: TimeDisplayProps) => {
  ...
  switch (clockDisplayMode) {
    case ClockDisplayMode.DIGITAL:
      return <DigitalClock hours={hours} minutes={minutes} />
    case ClockDisplayMode.ANALOG:
      return <AnalogClock hours={hours} minutes={minutes} />
  }
}

const DigitalClock = ({ hours, minutes }: { hours: string, minutes: string }) => {
  return (
...
const AnalogClock = ({ hours, minutes }: { hours: string, minutes: string }) => {
  const date = new Date()
